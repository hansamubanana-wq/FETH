...
export class Race3DRenderer {
    ...
    _updateCamera(distances, dt) {
        ...
        // 平滑化による追従遅れを確定後の姿勢で再検査する。
        // ズームアウトだけ即時に行い、全馬を画面端から8%内側に保つ。
        let maxAbsR = 0, maxAbsU = 0;
        for (let i = 0; i < distances.length; i++) {
            const pose = this._pose(distances[i], this.layout.off[i]);
            for (const headY of [0, LABEL_HEADROOM]) {
                point.copy(pose.position).sub(this.viewTarget);
                point.y += headY;
                maxAbsR = Math.max(maxAbsR, Math.abs(point.dot(right)) + VIEW_MARGIN_X);
                maxAbsU = Math.max(maxAbsU, Math.abs(point.dot(up)) + VIEW_MARGIN_Y);
            }
        }
        const safeHeight = Math.max(
            maxAbsU * 2 / SAFE_FRAME_NDC,
            maxAbsR * 2 / (this.aspect * SAFE_FRAME_NDC)
        );
        this.viewHeight = Math.max(this.viewHeight, safeHeight);
        this._applyFrustum();
    }
